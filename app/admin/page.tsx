...
export default function AdminPage() {
  ...
  async function saveDraft() {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus(`Opslaan faalde (${res.status}): ${data?.error ?? "onbekend"}`);
        return;
      }
  ...
  async function publish() {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus(`Publiceren faalde (${res.status}): ${data?.error ?? "onbekend"}`);
        return;
      }
  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 22, fontSize: 12, color: "#666" }}>
        Tip: plak het assessmentId uit de student view. Open daarna in een tweede tab de studentpagina en druk op “Ververs docentfeedback”.
      </div>
    </div>
  );
}
